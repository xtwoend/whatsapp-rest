...
import (
	...
	"github.com/xtwoend/whatsapp-rest/pkg/log"
...
// TODO read message & call web hook
...
func (wah *waHandler) HandleImageMessage(data whatsapp.ImageMessage)  {
	...
	if ! data.Info.FromMe && data.Info.Timestamp >= wah.SessionStart {
		...
		reqBody := map[string]interface{}{
			...
			"image_url": "http://" + server.Config.GetString("SERVER_IP") + ":" + server.Config.GetString("SERVER_PORT") + "/media/" + filename,
		}
...
func (wah *waHandler) HandleVideoMessage(data whatsapp.VideoMessage)  {
	...
	if ! data.Info.FromMe && data.Info.Timestamp >= wah.SessionStart {
		...
		reqBody := map[string]interface{}{
			...
			"video_url": "http://" + server.Config.GetString("SERVER_IP") + ":" + server.Config.GetString("SERVER_PORT") + "/media/" + filename,
		}
...
func (wah *waHandler) HandleBatteryMessage(battery whatsapp.BatteryMessage) {
...
func callHook(message interface{}) {
	url := server.Config.GetString("WEBHOOK_URL")

	bytesRepresentation, err := json.Marshal(message)
	if err != nil {
		log.Println(log.LogLevelError, "whatsapp-message-handler", err)
	}
	...
	log.Println(log.LogLevelInfo, "whatsapp-message-handler", result)
}
...
func WaPing(jid string) (interface{}, error) {
	con := wac[jid]

	if con == nil {
		return nil, errors.New("something when wrong while trying to ping, please check phone connectivity")
	}
...
func WASessionInit(jid string, versionClientMajor int, versionClientMinor int, versionClientBuild int, timeout int) error {
	if wac[jid] == nil {
		...
		conn.SetClientName("Whatsapp Api", "Whatsapp Api", "1.0")
