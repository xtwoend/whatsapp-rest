...
import (
	...
	"github.com/xtwoend/whatsapp-rest/pkg/whatsapp"
)
...
func WhatsAppLogin(w http.ResponseWriter, r *http.Request) {
	...
	jid := r.FormValue("jid")
	if len(jid) == 0 {
		router.ResponseInternalError(w, "Jid required")
		return
	}
	...
	case qrcode := <-qrstr:
		...
		case "json":
			...
			response.Data.Reconnect = reqBody.Reconnect
...
func WhatsAppPing(w http.ResponseWriter, r *http.Request)  {
	...
	router.ResponseSuccessWithData(w, "", info)
}